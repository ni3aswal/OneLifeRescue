...
if(isset($resultsForHashTag->search_metadata->next_results)){      
        //Parsing information for max_id in tweets fetched
        $string="?max_id="; 
        $parse=explode("&",$resultsForHashTag->search_metadata->next_results);       
        $maxID=substr($parse[0],strpos($parse[0],$string)+strlen($string));     
		$maxidHashTag = -1+$maxID;
}
else
{
	$maxidHashTag =$twitterMaxID;
}
...
if(isset($resultsForUser->search_metadata->next_results)){      
        //Parsing information for max_id in tweets fetched
        $string="?max_id="; 
        $parse=explode("&",$resultsForUser->search_metadata->next_results);       
        $maxID=substr($parse[0],strpos($parse[0],$string)+strlen($string));     
		$maxidHashTag = -1+$maxID;
