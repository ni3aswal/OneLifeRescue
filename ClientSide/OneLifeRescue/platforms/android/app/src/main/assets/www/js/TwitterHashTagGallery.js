...
;(function ($) {
	...
	$.fn.loadTweets = function (settings) {
		...
		var _self = document.getElementById('content1');
		/**
		 * Show the loading image
		 */
		var showLoader = function () {
			if($('#head1 h1 span').next().is('img')){
				$('#head1 h1 span').next('img').show();
			}else{
				$('#head1 h1 span').after($('<img />', {
		...
		var _init = function () {
			...
			$.getJSON('http://192.168.43.14/scripts/get-tweets.php?', $.param(data), function (feeds) {
		...
		var _success = function (feeds) {
			...
			for (var i = 0; i < feeds.length; i++) {
				...
				if (((config.showReTweets == true) || ((isaretweet == false) && (config.showReTweets == false))) && ((config.showDirectTweets == true) || ((config.showDirectTweets == false) && (isdirect == false)))) {
					if ((feeds[i].text.length > 1) && (displayCounter <= config.twitterLimitCount)) {
						...
						feedHTML += '<div class="twitter-article" id="tw' + displayCounter + '"><div class="twitter-pic"><a href="https://twitter.com/' + tweetusername + 
						'" target="_blank"><img src="' + profileimage + '"images/twitter-feed-icon.png" width="42" height="42" alt="twitter icon" /></a></div><div class="twitter-text"><p><span class="tweetprofilelink"><strong><a href="https://twitter.com/' + tweetusername + '" target="_blank">' + tweettwitterScreenName + 
						'</a></strong><a href="https://twitter.com/' + tweetusername + '" target="_blank">@' + tweetusername + 
						'</a></span><span class="tweet-time"><a href="https://twitter.com/' + tweetusername + '/status/' + 
						tweetid + '" target="_blank">' + _time(feeds[i].created_at) + '</a></span><br/>' + status + '</p>';
			...
			_self.innerHTML=_self.innerHTML+feedHTML;
